...
export type ConfiguredObjectClassForName<Name extends PlaceableDocumentType> =
	CONFIG[Name]['objectClass']

type SubtypeData<
	SubType,
	SubTypeValue extends SubTypeShape | undefined
> = SubTypeValue extends undefined
	? Record<string, unknown>
	: ([GetKey<SubTypeValue, 'model'>] extends [undefined]
			? GetKey<SubTypeValue, 'data', never>
			: DataModel.SchemaToData<
					GetKey<GetKey<SubTypeValue, 'model'>, 'schema'>
			  >) & {
			type: SubType
	  }
...
export type ConfiguredSheetClass<T extends Metadata<any>> =
	T['name'] extends keyof CONFIG
		? 'sheetClass' extends keyof CONFIG[T['name']]
			? CONFIG[T['name']]['sheetClass']
			: never
		: T

export type ObjectClass<T extends DocumentConstructor> =
...
export type DataSourceForPlaceable<P extends PlaceableObject> =
	P extends PlaceableObject<infer Doc> ? Doc['_source'] : never
 ...
 */
export const StructuralClass: new <
